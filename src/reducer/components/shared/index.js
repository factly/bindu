...
import {SET_AREA_LINES, SET_AREA_LINE_WIDTH, SET_AREA_LINE_OPACITY, SET_AREA_LINE_CURVE, SET_AREA_LINE_DASHED}  from "../../../constants/area_lines.js";
import {SET_BAR_OPACITY, SET_BAR_CORNER_RADIUS} from "../../../constants/bars.js";
import {SET_TITLE, SET_WIDTH, SET_HEIGHT, SET_BACKGROUND} from "../../../constants/chart_properties.js";
import {SET_COLOR} from "../../../constants/colors.js";
import {SET_DATA_LABELS, SET_DATA_LABELS_COLOR, SET_DATA_LABELS_SIZE, SET_DATA_LABELS_FORMAT} from "../../../constants/data_labels.js";
...
import {SET_LEGEND_POSITION, SET_LEGEND_TITLE, SET_LEGEND_BACKGROUND, SET_LEGEND_SYMBOL, SET_LEGEND_SYMBOL_SIZE} from "../../../constants/legend.js";
import {SET_LEGEND_LABEL_POSITION, SET_LEGEND_LABEL_BASELINE, SET_LEGEND_LABEL_COLOR} from "../../../constants/legend_label.js";
import {SET_LINE_WIDTH, SET_LINE_OPACITY, SET_LINE_CURVE, SET_LINE_DASHED} from "../../../constants/lines.js";
...
import {SET_XAXIS_TITLE, SET_XAXIS_POSITION, SET_XAXIS_LABEL_FORMAT, SET_XAXIS_LABEL_COLOR} from "../../../constants/x_axis.js";
...
const sharedReducer = (state = {}, action) => {
  ...
  switch (action.type) {
    ...
    case SET_BACKGROUND:
      return produce(state, draftState => {
          const {path, value} = payload;
          const backgroundObj = getValueFromNestedPath(draftState.spec, path.slice(0, path.length - 1));
    ...
    case SET_XAXIS_TITLE:
      ...
      return produce(state, draftState => {
          const {path, value} = payload;
          const xaxisTitleObj = getValueFromNestedPath(draftState.spec, path.slice(0, path.length - 1));
    ...
    case SET_XAXIS_POSITION:
      ...
      return produce(state, draftState => {
          const {path, value} = payload;
          const xaxisPositionObj = getValueFromNestedPath(draftState.spec, path.slice(0, path.length - 1));
    ...
    case SET_XAXIS_LABEL_FORMAT:
      ...
      return produce(state, draftState => {
          const {path, value} = payload;
          const labelFormatObj = getValueFromNestedPath(draftState.spec, path.slice(0, path.length - 1));
    ...
    case SET_XAXIS_LABEL_COLOR:
      ...
      return produce(state, draftState => {
          const {path, value} = payload;
          const labelColorObj = getValueFromNestedPath(draftState.spec, path.slice(0, path.length - 1));
    ...
    case SET_LEGEND_LABEL_BASELINE:
      ...
      return produce(state, draftState => {
        const {path, value} = payload;
        const labelBaselineObj = getValueFromNestedPath(draftState.spec, path.slice(0, path.length - 1));
    ...
    case SET_LEGEND_BACKGROUND:
      ...
      return produce(state, draftState => {
        const {path, value} = payload;
        const legendBgColorObj = getValueFromNestedPath(draftState.spec, path.slice(0, path.length - 1));
    ...
    case SET_LEGEND_SYMBOL:
      ...
      return produce(state, draftState => {
        const {path, value} = payload;
        const legendSymbolObj = getValueFromNestedPath(draftState.spec, path.slice(0, path.length - 1));
    ...
    case SET_LEGEND_SYMBOL_SIZE:
      ...
      return produce(state, draftState => {
        const {path, value} = payload;
        const symbolSizeObj = getValueFromNestedPath(draftState.spec, path.slice(0, path.length - 1));
    ...
    case SET_BAR_CORNER_RADIUS:
      return produce(state, draftState => {
          const {path, value} = payload;
          const cornerRadiusObj = getValueFromNestedPath(draftState.spec, path.slice(0, path.length - 1));
    ...
    case SET_PIE_OUTER_RADIUS:
      return produce(state, draftState => {
        const {path, value} = payload;
        const outerRadiusObj = getValueFromNestedPath(draftState.spec, path.slice(0, path.length - 1));
        outerRadiusObj[path[path.length - 1]] = parseInt(value);
      });
    case SET_PIE_INNER_RADIUS:
      return produce(state, draftState => {
        const {path, value} = payload;
        const innerRadiusObj = getValueFromNestedPath(draftState.spec, path.slice(0, path.length - 1));
    ...
    case SET_PIE_PADDING_ANGLE:
      return produce(state, draftState => {
        const {path, value} = payload;
        const paddingAngleObj = getValueFromNestedPath(draftState.spec, path.slice(0, path.length - 1));
