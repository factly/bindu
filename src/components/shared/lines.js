  ...
  return (
    <div className="property-container">
    	<Row gutter={[0, 12]}>
			...
			<Col span={12}>
				<Input value={strokeWidth} type="number" onChange={(e) => dispatch({type: SET_LINE_WIDTH, payload: {value: e.target.value, path: strokeWidthObj.path}, chart: "shared"})} />
			</Col>
		</Row>
		<Row gutter={[0, 12]}>
			<Col span={12}>
			...
			<Col span={12}>
				<Input value={opacity} min={0} max={1} step={0.05} type="number" onChange={(e) => dispatch({type: SET_LINE_OPACITY, payload: {value: e.target.value, path: opacityObj.path}, chart: "shared"})} />
			</Col>
		</Row>
		<Row gutter={[0, 12]}>
			<Col span={12}>
			...
			<Col span={12}>
				<Select value={interpolate} onChange = {(value) => dispatch({type: SET_LINE_CURVE, payload: {value: value, path: interpolateObj.path}, chart: "shared"})}>
		...
		<Row gutter={[0, 12]}>
			...
			<Col span={12}>
				<Input value={strokeDash} type="number" onChange={(e) => dispatch({type: SET_LINE_DASHED, payload: {value: e.target.value, path: strokeDashObj.path}, chart: "shared"})} />
			</Col>
		</Row>
    </div>
  );
